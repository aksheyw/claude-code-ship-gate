...
[![License: MIT](https://img.shields.io/badge/license-MIT-green.svg)](LICENSE) ![tests: 265 passing](https://img.shields.io/badge/tests-265%20passing-brightgreen.svg) ![Claude Code plugin](https://img.shields.io/badge/Claude%20Code-plugin-8A2BE2.svg)
...
| Deterministic runner | Script | `scripts/ship-gate.sh detect\|run\|write-marker\|protected-branch\|disable\|enable\|status` — runs the gates, resolves the protected branch, toggles per-repo gating; CI-reusable. |
| JSON Schema | Config | `schema/shipgate.schema.json` (Draft-07) for editor autocomplete on `.shipgate.json`. |

Everything deterministic lives in dependency-free **bash + jq + awk**; the skills hold prompt logic only. The full test suite is 265 assertions across `scripts/test/*_test.sh`, no framework required.
...
| `gates.*.command` | `null` (auto-detect) | Override the auto-detected command for any deterministic gate. |
...
| `regression.enabled` | `false` | Enable AI regression testing on test-affecting changes. |
...
| regression | only when `regression.enabled` **and** the change is test-affecting |
...
| regression | (disabled by default) | `ai-regression-testing` |
...
```sh
# 1. Full deterministic test suite (expect TOTAL PASS=265 FAIL=0)
