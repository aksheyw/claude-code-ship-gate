...
# Resolve the marker via the per-worktree git-dir (anchored to $REPO_ROOT so a relative ".git" joins
# correctly even when the hook CWD is a repo subdir; absolute in a linked worktree). NOT $REPO_ROOT/.git,
# which is a FILE in a linked worktree (P8-7).
_GDIR=$(git -C "$REPO_ROOT" rev-parse --git-dir 2>/dev/null || echo "$REPO_ROOT/.git")
case "$_GDIR" in /*) : ;; *) _GDIR="$REPO_ROOT/$_GDIR" ;; esac
MARKER="$_GDIR/shipgate/last-pass.json"
[ -f "$MARKER" ] || deny "No ship-gate pass. Run /ship before pushing to ${MAIN}."

M_HEAD=$(jq -r '.head // empty' "$MARKER")
M_TS=$(jq -r '.ts // empty' "$MARKER")

# Guard: empty HEAD in marker is corrupt
[ -n "$M_HEAD" ] || deny "ship-gate marker is corrupt. Re-run /ship."

# Guard: get current HEAD safely
CUR_HEAD=$(git rev-parse HEAD 2>/dev/null || echo "")
[ -n "$CUR_HEAD" ] || deny "ship-gate: unable to determine current HEAD."

[ "$M_HEAD" = "$CUR_HEAD" ] || deny "ship-gate pass is for a different commit. Re-run /ship."

# Guard: M_TS must be a non-empty non-negative integer to avoid arithmetic crash
if [ -z "$M_TS" ] || ! printf '%s' "$M_TS" | grep -qE '^[0-9]+$'; then
  deny "ship-gate marker is corrupt. Re-run /ship."
fi

NOW=$(date +%s)
[ $((NOW - M_TS)) -le "$TTL" ] || deny "ship-gate pass expired (>${TTL}s). Re-run /ship."

# --- Validate marker.branch matches push target ---
M_BRANCH=$(jq -r '.branch // empty' "$MARKER")
[ "$M_BRANCH" = "$TARGET" ] || deny "ship-gate pass is for branch '${M_BRANCH}', not '${TARGET}'. Re-run /ship."
